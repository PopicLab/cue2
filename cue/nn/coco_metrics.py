...
class CocoKeypointEvaluator:
    def __init__(self, classes, output_dir):
        ...
        self.coco_eval.params.catIds = range(1, len(classes))
        self.coco_eval.params.kpt_oks_sigmas = np.array([.5])
    ...
    def plot_roc(self):
        ...
        for i, area_label in enumerate(self.coco_eval.params.areaRngLbl):
            ...
            area_precision = self.coco_eval.eval['precision'][..., i, 0]
            p_mean = [p.mean() for p in area_precision]
            p_curve = [p.mean(axis=1) if p.ndim > 1 else p for p in area_precision]
            p_curve.insert(0, np.zeros(p_curve[0].shape))
            for t, iou_thr in enumerate(iou_thresholds):
                axs[i].plot(self.coco_eval.params.recThrs, p_curve[t + 1], color=color_map(t), marker='o', linewidth=0.5,
                            label="IoU=%.2f:%.3f" % (iou_thr, p_mean[t]))
            axs[i].legend()
...
def convert_results_to_coco(outputs):
    ...
    for output in outputs:
        if len(output["keypoints"]) == 0: continue
        scores = output["scores"].tolist()
        labels = output["labels"].tolist()
        keypoints = output["keypoints"].flatten(start_dim=1).tolist()
...
def convert_to_coco(batch):
    ...
    for image, target in batch:
        image_id = target[TargetType.image_id].item()
        img_dict = {'id': image_id, 'height': image.shape[-2], 'width': image.shape[-1]}
        dataset['images'].append(img_dict)
        if TargetType.labels not in target or target[TargetType.labels].numel() == 0:
            empty_image_ids.add(image_id)
            continue
        bboxes = copy.deepcopy(target[TargetType.boxes])
        areas = (bboxes[:, 3] - bboxes[:, 1]) * (bboxes[:, 2] - bboxes[:, 0])  # TODO: fix by 1
        bboxes[:, 2:] -= bboxes[:, :2]
        bboxes = bboxes.tolist()
        labels = target[TargetType.labels].tolist()
        areas = areas.tolist()
        keypoints = target[TargetType.keypoints]
