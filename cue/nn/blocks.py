...
class Conv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=0, pool=True, relu=True, bn=False):
        ...
        layers = [nn.Conv2d(in_channels=in_channels,
                            ...
                            stride=stride,
                            padding=padding)]
        if pool: layers.append(nn.MaxPool2d(kernel_size=2, stride=2, padding=0))
...
class Hourglass(nn.Module):
    def __init__(self, depth, nc, expansion):
        ...
        if self.depth > 1: self.low2 = Hourglass(self.depth - 1, nc_expanded, expansion)
        else: self.low2 = Residual(nc_expanded, nc_expanded)
