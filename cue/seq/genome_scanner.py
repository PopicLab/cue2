# Copyright (c) 20242025 Victoria Popic
#
...
from seq.constants import SIGNAL_TYPE, SVSignals
from seq.intervals import GenomeInterval, GenomeIntervalPair
from seq.io import SVContainer
...
class TargetIntervalScanner(GenomeScanner):
    def __init__(self, config, aln_index):
        ...
        self.config = config
        self.high_redundancy = self.config.high_redundancy
        ...
        if self.config.fixed_targets:
            ...
            for sv in SVContainer(self.config.fixed_targets, aln_index.chr_index):
                ...
                self.record_interval_pair_support(sv.start, sv.end, SIGNAL_TYPE.DIRECT_ADJ, 1000)
        else:
        ...
        logging.info("Number of target interval pairs for %s: %d" % (self.chr.name, len(self.interval_pairs)))
    ...
    def select_interval_pairs(self):
        ...
        for interval_pair in sorted(self.interval_pair_support.keys()):
            ...
            for i, group in enumerate([[SVSignals.DEL, SVSignals.SRC],
                                       [SVSignals.SRS, SVSignals.SPS],
                                       [SVSignals.SRI, SVSignals.SPI]]):
                for signal in group:
        ...
        logging.info("Selected %d interval pairs out of %d pairs" % (len(self.interval_pairs),
                                                                     len(self.interval_pair_support)))
